# -*- coding: utf-8 -*-
"""
密码/任意文本 <-> BIP39助记词 无损转换工具（支持可选加密）
依赖: pip install cryptography
用法:
  python main.py --encode "文本"
  python main.py --decode "助记词"
  python main.py --encode "文本" --key "你的密钥"
  python main.py --decode "助记词" --key "你的密钥"
"""
...
# 需额外安装: pip install cryptography
from cryptography.hazmat.primitives.kdf.pbkdf2 import PBKDF2HMAC
from cryptography.hazmat.primitives import hashes
from cryptography.hazmat.primitives.ciphers.aead import AESGCM

# 📦 BIP39 英文标准词表 (2048词)
BIP39_WORDS = """abandon ability able about above absent absorb abstract absurd abuse access accident
account accuse achieve acid acoustic acquire across act action actor actress actual
addict address adjust admit adult advance advice aerobic affair afford afraid again
age agency agenda agent agile aging agree ahead aim air airport aisle alarm album
...
apple approve april arch arctic area arena argue arm armed armor army arrange
...
beef before begin behave behind believe below bench benefit best betray better
between beyond bicycle bid bike bind biology bird birth bitter black blade blame
blanket blast bleak bless blind blood blossom blow blue blur blush board boat
body boil bomb bone bonus book boost border boring borrow boss bottom bounce
box boy bracket brain brand brass brave bread breeze brick bridge brief bright
bring brisk broccoli broken bronze broom brother brown brush bubble buddy budget
buffalo build bulb bulk bullet bundle bunny burden burger burst bus business busy
butter buyer buzz cabbage cabin cable cactus cage cake call calm camera camp can
...
cereal certain chair chalk champion change chaos chapter charge chase cheap check
cheese chef cherry chest chicken chief child chimney choice choose chronic chuckle
chunk churn citizen city civil claim clap clarify claw clay clean clerk clever
...
employ empower empty enable enact end endless energy enemy enforce engage engine
enhance enjoy enlist enough enrich enroll ensure enter entire entry envelope
episode equal equip erase erode erosion error erupt escape essay essence estate
...
female fence festival fetch fever few fiber fiction field figure file film filter
final find fine finger finish fire firm fiscal fish fit fitness fix flag flame
...
galaxy gallery game gap garage garbage garden garlic garment gas gasp gate gather
...
grab grace grain grant grape grass gravity great green grid grief grit grocery
group grow grunt guard guess guide guilt guitar gun gym habit hair half hammer
hamster hand happy harbor hard harsh harvest hat have hawk hazard head health
heart heavy hedgehog height hello helmet help hen hero hip hire history hobby
...
increase index indicate indoor industry infant inflict inform initial inject
...
jump jungle junior junk just kangaroo keen keep ketchup key kick kid kidney kind
king kit kitchen kite kitten kiwi knee knife knock know lab label labor ladder
lady lake lamp language laptop large later latin laugh laundry lava lawn lawsuit
layer lazy leader leaf learn leave lecture left leg legal legend leisure lemon
...
lumber lunar lunch luxury lyrics machine mad magic magnet maid mail main major
...
milk million mimic mind minimum minor minute miracle mirror misery miss mistake
...
myth naive name napkin narrow nasty nation nature near neck need negative neglect
neither nephew nerve nest net network neutral never news next nice night noble
noise nominee noodle normal north nose notable nothing notice novel now nuclear
...
pizza place planet plastic plate play please pledge pluck plug plunge poem point
...
produce profit program promote proof property prosper protect proud provide public
...
rabbit raccoon race rack radar radio rage rail rain raise rally ramp ranch random
range rapid rare rate rather raven raw razor ready real reason rebel rebuild recall
receive recipe record recycle reduce reflect reform region regret regular reject
relax release relief rely remain remember remind remove render renew rent reopen
repair repeat replace report require rescue resemble resist resource response
result retire retreat return reunion reveal review reward rhythm rib ribbon rice
rich ride ridge rifle right rigid ring riot ripple risk ritual rival river road
roast robot robust rocket romance roof rookie room rose rotate rough round route
royal rubber rude rug rule run runway rural sad saddle safety sail salad salmon
...
spirit split sponsor spoon sport spot spray spread spring spy square squeeze
squirrel stable stadium staff stage stairs stamp stand start state stay steak steel
stem step stereo stick still sting stock stomach stone stool story strategy street
strike strong struggle student stuff stumble style subject submit subway success
such sudden suffer sugar suggest suit summer sun sunny sunrise super supply supreme
sure surface surge surprise surround survey suspect sustain swallow swamp swap
swarm swear sweet swim swing switch sword symbol symptom syrup system table tackle
...
tiny tip tired tissue title toast today toddler toe together toilet token tomato
tomorrow tone tongue tonight tool top topic topple torch tornado tortoise toss
...
unable uncle under undo unfair unfold unhappy uniform unique unit universe unknown
unlock until unusual unveil update upgrade uphold upon upper upset urban usage use
...
vote voyage wage wait walk wall walnut want warfare warm warrior wash wasp waste
...
yard year yellow you young youth zebra zero zone zoo""".strip().split()


class BIP39Codec:
    def __init__(self, wordlist: list = BIP39_WORDS):
        self.wordlist = wordlist
        self.word_to_idx = {w: i for i, w in enumerate(wordlist)}
        self.base = len(wordlist)

    def to_mnemonic(self, data: bytes) -> str:
        payload = len(data).to_bytes(4, 'big') + data
        padded = b'\x01' + payload
        num = int.from_bytes(padded, 'big')
        words = []
        while num > 0:
            words.append(self.wordlist[num % self.base])
            num //= self.base
        return ' '.join(reversed(words)) if words else self.wordlist[0]

    def from_mnemonic(self, mnemonic: str) -> bytes:
        if not mnemonic.strip(): return b""
        words = mnemonic.strip().lower().split()
        num = 0
        for w in words:
            if w not in self.word_to_idx:
                raise ValueError(f"非标准BIP39词: '{w}'")
            num = num * self.base + self.word_to_idx[w]
        byte_len = (num.bit_length() + 7) // 8
        data = num.to_bytes(byte_len, 'big')
        if data[0] != 1: raise ValueError("数据损坏：校验前缀缺失")
        payload = data[1:]
        if len(payload) < 4: raise ValueError("数据损坏：长度头不足")
        pwd_len = int.from_bytes(payload[:4], 'big')
        if len(payload) < 4 + pwd_len: raise ValueError("数据不完整或已损坏")
        return payload[4:4+pwd_len]


class CryptoWrapper:
    """AES-GCM 认证加密 (依赖 cryptography)"""
    @staticmethod
    ...
    def encrypt(plaintext: str, key: str) -> bytes:
        salt = os.urandom(16)
        nonce = os.urandom(12)
        derived = CryptoWrapper._derive_key(key, salt)
        ct = AESGCM(derived).encrypt(nonce, plaintext.encode(), None)
        return salt + nonce + ct

    @staticmethod
    def decrypt(ciphertext: bytes, key: str) -> str:
        if len(ciphertext) < 28: raise ValueError("密文长度不足")
        salt, nonce, ct = ciphertext[:16], ciphertext[16:28], ciphertext[28:]
        derived = CryptoWrapper._derive_key(key, salt)
        return AESGCM(derived).decrypt(nonce, ct, None).decode()


def main():
    if sys.stdout.encoding != 'utf-8': sys.stdout.reconfigure(encoding='utf-8')
    parser = argparse.ArgumentParser(description="密码/文本 <-> BIP39助记词 可逆转换（支持加密）")
    parser.add_argument("--encode", type=str, help="待编码的原始文本")
    parser.add_argument("--decode", type=str, help="待解码的助记词")
    parser.add_argument("--key", type=str, help="加密/解密密钥（可选，提供则启用AES-GCM加密）")
    ...
    try:
        if args.encode:
            data = args.encode.encode() if not args.key else CryptoWrapper.encrypt(args.encode, args.key)
            print(codec.to_mnemonic(data))
        elif args.decode:
            raw_bytes = codec.from_mnemonic(args.decode)
            result = raw_bytes.decode() if not args.key else CryptoWrapper.decrypt(raw_bytes, args.key)
            print(result)
